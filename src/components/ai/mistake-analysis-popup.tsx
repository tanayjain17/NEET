...
export default function MistakeAnalysisPopup({ 
  ...
  const handleSubmit = async () => {
    if (selectedCategories.length === 0) {
      alert('Data Input Required: Please categorize your session results.')
      return
    }
    ...
    if (sessionType === 'test' && selectedCategories.includes('no_mistakes') && sessionData.testScore && sessionData.testScore < 710) {
      alert('⚠️ Validation Error: "Zero Errors" is statistically improbable for scores < 710. Please log specific error types.')
      return
    }
    ...
    const mistakeData: MistakeData = {
      ...
      specificMistakes: selectedCategories.includes('no_mistakes') ? ['Optimal performance achieved'] : specificMistakes.filter(m => m.trim() !== ''),
      improvementAreas: selectedCategories.includes('no_mistakes') ? ['Protocol Maintenance'] : improvementAreas,
  ...
  const handleSkip = () => {
    alert('⚠️ Mandatory Field: Performance data is required for the AI Prediction Engine.')
  }
  ...
  const toggleCategory = (categoryId: string) => {
    setSelectedCategories(prev => {
      const has = prev.includes(categoryId)
      
      // If selecting "no_mistakes", clear everything else
      if (categoryId === 'no_mistakes') {
        return has ? [] : ['no_mistakes']
      }
      
      // If selecting something else, ensure "no_mistakes" is removed
      const withoutNoMistakes = prev.filter(id => id !== 'no_mistakes')
      return has 
        ? withoutNoMistakes.filter(id => id !== categoryId) 
        : [...withoutNoMistakes, categoryId]
    })
  }
              ...
              <CardContent className="space-y-8 pt-6">
                ...
                <div>
                  ...
                  {sessionType === 'test' && sessionData.testScore && sessionData.testScore < 710 && (
                    <div className="mb-4 p-3 bg-rose-500/10 border border-rose-500/20 rounded-lg flex gap-3 items-center">
                      <AlertCircle className="h-5 w-5 text-rose-400" />
                      <p className="text-rose-200 text-xs">
                        Anomaly Check: 'Zero Errors' not permitted for scores below 710. Precise categorization required for AI calibration.
                      </p>
                    </div>
                  )}
                ...
                {/* Logic to hide details if "No Mistakes" is selected */}
                {!selectedCategories.includes('no_mistakes') && (
                  <>
                    {/* Specific Mistakes */}
                    <div>
                      <h3 className="text-md font-semibold text-white mb-4">Error Logs (Details)</h3>
                ...
                <div className="flex justify-end space-x-4 pt-4 border-t border-white/5">
                  <button
                    onClick={handleSkip}
                    className="px-6 py-3 border border-rose-500/30 text-rose-400 rounded-xl hover:bg-rose-500/10 transition-colors text-sm font-medium"
                  >
                    Skip (Restricted)
                  </button>
                  <button
                    ...
                    disabled={loading}
                    className="px-8 py-3 bg-gradient-to-r from-blue-600 to-cyan-600 text-white rounded-xl font-semibold shadow-lg shadow-cyan-500/20 hover:shadow-cyan-500/40 hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50 disabled:scale-100 flex items-center gap-2"
                  >
                    {loading ? (
                      ...
                      <>
                        <Brain className="h-4 w-4" /> Finalize Analysis
                      </>
                    )}
                  </button>
                </div>
